...
class SponsorScannerTests(unittest.TestCase):
    ...
    def test_explicit_sponsor_and_domain_are_extracted(self) -> None:
        video = self._video(
            "Thanks to NordVPN for sponsoring today's video!\n"
            "Get the deal here: https://nordvpn.com/creator #ad"
        )
        ...
        self.assertTrue(detections[0].explicit_phrase)

    def test_creator_is_classified_as_gaming(self) -> None:
        video = self._video("Minecraft survival challenge with friends")
        channel = ChannelRecord(
            channel_id="creator-channel",
            title="Gaming Creator",
            description="Minecraft and gaming videos every week",
    ...
    def test_existing_domain_blocks_repeat_brand(self) -> None:
        video = self._video(
