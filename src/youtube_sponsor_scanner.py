...
class YouTubeSponsorScanner:
    ...
    def _get(self, endpoint: str, params: dict) -> dict:
        ...
        response = self.session.get(f"{YOUTUBE_API}/{endpoint}", params=request_params, timeout=30)
    ...
    def discover_video_ids(self, lookback_hours: int) -> list[str]:
        # Strongest source first: videos where the uploader explicitly marked paid promotion.
        ordered_ids: list[str] = []
        seen: set[str] = set()

        searches = [("", True)] + [(term, False) for term in SEARCH_TERMS]
        for query, paid_only in searches:
            try:
                ids = self._search(lookback_hours, query=query, paid_only=paid_only)
            except RuntimeError as exc:
                # One query should not kill the whole daily run.
    ...
    def fetch_videos(self, video_ids: list[str]) -> list[VideoRecord]:
        ...
        for start in range(0, len(video_ids), 50):
            ...
            try:
                ...
                # brandPartner can be unavailable in some API contexts. The scanner still works
                # from the public paid-placement flag and description signals.
